...
const getFields = ({ fields }, allowedFieldNames = fields.map(defaultField => defaultField.name)) => fields.filter(({ name }) => allowedFieldNames.includes(name));
        ...
        ({ data: api, loading, error }) => {
          ...
          return (
            <List {...props}>
              <Datagrid>
                {
                  getFields(resource, allowedFieldNames).map(
                    (field) => {
